...
- **User asks for batch generation** ("generate all missing propositions") → Action-oriented with a gate. Get to the priority list fast — the user wants output, not a consulting session. Open with a one-line portfolio snapshot ("4 features x 2 markets = 8 pairs, 2 exist, 6 missing"). Present your tiered priority list immediately. If existing propositions have quality issues, flag them briefly in one line ("your 2 existing propositions also need rework — I'll address those after we confirm priorities") — do NOT do a full critique here, that's the review path. End with the confirmation gate: "Confirm this priority and I'll start drafting." The batch response should reach the gate within the first half of the response.
